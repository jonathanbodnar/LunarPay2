...
class Ion_auth_model extends CI_Model {
    ...
    public function register($identity, $password, $email, $additional_data = [], $groups = []) {
        ...
        if(isset($id) && !isset($user_data['parent_id'])){//====== if parent id is set it means an internal user creation was performed we do not have to set sessions
            $query = $this->db->select($this->identity_column . ', zelle_account_id, referral_code, email, id, password, active, last_login, first_name, last_name, created_on, parent_id, permissions, payment_processor, role')
                ->where($this->identity_column, $identity)
                ->limit(1)
                ->order_by('id', 'desc')
                ->get($this->tables['users']);
    ...
    public function process_referrals($data = []){

        if(count($data)!=0){

            $query = $this->db->select('id') //if code ref exist
            ->where('referral_code', $data['referral_code'])
            ->limit(1)
            ->get($this->tables['users']);

             
            if ($query->num_rows() === 0) {//ref does not exist
                return FALSE;
            }

            $query2 = $this->db->select('id') //if email was sent
            ->where('email', $data['email'])
            ->limit(1)
            ->get('referals');
            
            if ($query->num_rows() === 1 && $query2->num_rows() === 1) {
                $ref_id = $query2->row()->id;
                $this->db->update('referals', array('user_id'=>$data['user_id'],'date_register'=>date('Y-m-d') ), ['id' => $ref_id ]);
            
            }else if($query->num_rows() === 1 && $query2->num_rows() === 0) {
                $user_id = $query->row()->id;
                $this->db->insert('referals', array('parent_id'=>$user_id,'user_id'=>$data['user_id'],'email'=>$data['email'], 'full_name'=>$data['full_name'],'date_register'=>date('Y-m-d'),'date_sent'=>date('Y-m-d')));
            } 
    ...
    //we use parent_id for removing teamm members securely //$parent_id is the session user id
    public function delete_user($id, $parent_id = false) {        
        
        //It is checked that the user to be deleted is not the mime of the session.
        if ($parent_id) {
            $usersesion_id = $this->session->userdata('is_child') ? $this->session->userdata('child_id') : $parent_id;
            if ($usersesion_id == $id) {
                return ["status" => True, 'message' => "Error, you can't delete yourself"];
            }
        }
        ...
        $this->db->where('parent_id', $parent_id)
                ->delete($this->tables['users'], ['id' => $id]);

        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            $this->trigger_events(['post_delete_user', 'post_delete_user_unsuccessful']);
            $this->set_error('delete_unsuccessful');
            return ["status" => false, 'message' => 'Error trying to remove user'];
        }
        ...
        return ["status" => true, 'message' => 'User successfully removed'];
    }
    ...
    public function set_session($user, $opt1 = false) {
        ...
        $session_data = [
            ...
            'is_affiliate' => !$user->referral_code ? 0 : 1,
            'role' => $user->role
        ];
    ...
    public function login_remembered_user() {
        ...
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ',zelle_account_id, referral_code, id, email, remember_code, last_login, first_name, last_name, created_on, parent_id, permissions, payment_processor, role')
