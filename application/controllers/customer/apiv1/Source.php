    ...
    public function remove(){
        try {
            ...
            if($this->is_customer_request) {                
                $orgnx_id = null;
                $account_donor_id = $this->api_session_model->getValue($this->session_id, 'user_id');                
            } else {
                $account_donor_id = null;
                $orgnx_id = $this->orgnx_id; //we can provide the organization id instead de donor_id to secure removePaymentSource() function
            }

            require_once 'application/controllers/extensions/Payments.php';
            $pResult = Payments::removePaymentSource($sourceId, $account_donor_id, $orgnx_id);
